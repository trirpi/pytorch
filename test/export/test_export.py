...
def forward(self, p_linear_weight, p_linear_bias, b_buffer, x):
    ...
    def test_dynamic_shapes_inferred_basic(self):
        ...
        expect_error(
            ...
            run_time_msg=escape("Guard failed: x.size()[0] >= 3"),  # expected >= 3, but got 2
        ...
        expect_error(
            ...
            run_time_msg=escape("Guard failed: y[0].size()[0] == x.size()[0]"),  # expected 6, but got 7
        ...
        expect_error(
            ...
            run_time_msg=escape("Guard failed: z['k'].size()[0] == 3"),  # expected 3, but got 4
    ...
    def test_export_max_onnx_reported(self):
        ...
        with self.assertRaisesRegex(
            ...
            (
                ...
                else escape("Guard failed: max(1, x.size()[1], y.size()[1]) == x.size()[1]")
            ),
        ):
            # TODO: this should not error?
...
graph():
    ...
    def test_symint_input_basic(self):
        ...
        with self.assertRaisesRegex(AssertionError, escape("Guard failed: x == 4"),):
            # expected 4, but got 3
        ...
        with self.assertRaisesRegex(AssertionError, escape("Guard failed: x == 5"),):
    ...
    def test_disable_forced_specializations_ok(self):
        ...
        for private_api in (True, False):
            ...
            if private_api:
                ...
                with self.assertRaisesRegex(
                    AssertionError,
                    escape("Guard failed: x.size()[1] * x.size()[0] == y.size()[0] * y.size()[1]"),
                ):
    ...
    def test_dim_dynamic(self):
        ...
        for private_api in (True, False):
            ...
            if private_api:
                ...
                with self.assertRaisesRegex(
                    AssertionError,
                    escape("Guard failed: x.size()[1] * x.size()[0] % (-1 + x.size()[0]) == 0"),
                ):
