...
def _convert_guards_code_to_fn(guards_code, paths_of_placeholders):
    ...
    guards_fn = GuardsFn()
    guards_fn.forward = torch._dynamo.dont_skip_tracing(namespace["_"])  # type: ignore[call-overload]
...
def _check_input_constraints_pre_hook(self, args, kwargs):
    if hasattr(self, "_guards_fn") or not self.validate_inputs:
        return
...
def _unlift_exported_program_lifted_states(ep: ExportedProgram, check_guards=True) -> torch.fx.GraphModule:
    # TODO T206340015
    ...
    if check_guards and placeholders and ep.example_inputs:
        ...
        root_nn_module_stack = torch.fx._utils.first_call_function_nn_module_stack(graph)
